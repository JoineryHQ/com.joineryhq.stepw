<?php
use CRM_Stepw_ExtensionUtil as E;

return [
  [
    'name' => 'SavedSearch_StepwWorkflows',
    'entity' => 'SavedSearch',
    'cleanup' => 'unused',
    'update' => 'unmodified',
    'params' => [
      'version' => 4,
      'values' => [
        'name' => 'StepwWorkflows',
        'label' => E::ts('Stepwise Workflows'),
        'api_entity' => 'StepwWorkflow',
        'api_params' => [
          'version' => 4,
          'select' => [
            E::ts('id'),
            'title',
            'url',
            'is_active',
            'report_instance_id',
          ],
          'orderBy' => [],
          'where' => [],
          'groupBy' => [],
          'join' => [],
          'having' => [],
        ],
      ],
      'match' => [E::ts('name')],
    ],
  ],
  [
    'name' => 'SavedSearch_StepwWorkflows_SearchDisplay_StepwWorkflow_Search_by_a_example_com_Table_1',
    'entity' => 'SearchDisplay',
    'cleanup' => 'unused',
    'update' => 'unmodified',
    'params' => [
      'version' => 4,
      'values' => [
        'name' => 'StepwWorkflow_Search_by_a_example_com_Table_1',
        'label' => E::ts('StepwWorkflows Table1'),
        'saved_search_id.name' => 'StepwWorkflows',
        'type' => 'table',
        'settings' => [
          'description' => E::ts(''),
          'sort' => [],
          'limit' => 50,
          'pager' => [],
          'placeholder' => 5,
          'columns' => [
            [
              'type' => 'field',
              'key' => 'id',
              'dataType' => 'Integer',
              'label' => E::ts('ID'),
              'sortable' => TRUE,
            ],
            [
              'type' => 'field',
              'key' => 'title',
              'dataType' => 'String',
              'label' => E::ts('Title'),
              'sortable' => TRUE,
            ],
            [
              'type' => 'html',
              'key' => 'url',
              'dataType' => 'String',
              'label' => E::ts('Url'),
              'sortable' => TRUE,
              'rewrite' => '<span class="stepw-workflow-url">[url]</span><a class="stepw-workflow-url-copy action-item crm-hover-button" href="#copy"> <i class="crm-i fa-clipboard"></i> Copy URL</a>',
            ],
            [
              'type' => 'field',
              'key' => 'is_active',
              'dataType' => 'Boolean',
              'label' => E::ts('Enabled'),
              'sortable' => TRUE,
            ],
            [
              'type' => 'field',
              'key' => 'report_instance_id',
              'dataType' => 'Integer',
              'label' => E::ts('Report'),
              'sortable' => TRUE,
              'link' => [
                'path' => 'civicrm/report/instance/[report_instance_id]?force=1&reset=1',
                'entity' => '',
                'action' => '',
                'join' => '',
                'target' => '_blank',
              ],
              'rewrite' => '[report_instance_id.title]',
            ],
            [
              'text' => E::ts(''),
              'style' => 'default',
              'size' => 'btn-xs',
              'icon' => 'fa-bars',
              'links' => [
                [
                  'path' => '',
                  'icon' => 'fa-toggle-off',
                  'text' => E::ts('Disable'),
                  'style' => 'default',
                  'condition' => [E::ts('is_active'), '=', TRUE],
                  'task' => 'disable',
                  'entity' => 'StepwWorkflow',
                  'action' => '',
                  'join' => '',
                  'target' => 'crm-popup',
                ],
                [
                  'task' => 'enable',
                  'entity' => 'StepwWorkflow',
                  'join' => '',
                  'target' => 'crm-popup',
                  'icon' => 'fa-toggle-on',
                  'text' => E::ts('Enable'),
                  'style' => 'default',
                  'path' => '',
                  'action' => '',
                  'condition' => [E::ts('is_active'), '=', FALSE],
                ],
              ],
              'type' => 'menu',
              'alignment' => 'text-right',
            ],
          ],
          'actions' => [E::ts('disable'), 'enable'],
          'classes' => [E::ts('table'), 'table-striped'],
          'actions_display_mode' => 'menu',
          'cssRules' => [
            [E::ts('disabled'), 'is_active', '=', FALSE],
          ],
        ],
      ],
      'match' => [
        E::ts('saved_search_id'),
        'name',
      ],
    ],
  ],
];
