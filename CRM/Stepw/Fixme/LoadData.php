...
class CRM_Stepw_Fixme_LoadData {
  ...
  public static function getSampleData() {
    ...
    // Re-key $data to real workflow_ids; at this point, it's keyed to whatever
    // is in $localDataFile; but we want it keyed to the value of civicrm_stepw_workflow.id
    // for the corresponding workflow publicId.
    $liveData = [];
    $workflows = \Civi\Api4\StepwWorkflow::get()
      ->setCheckPermissions(FALSE)
      ->execute();
    $workflows = CRM_Utils_Array::rekey($workflows, 'public_id');
    foreach ($data as $dataWorkflowId => $workflowConfig) {
      $publicId = $workflowConfig['settings']['public_id'];
      $liveData[$workflows[$publicId]['id']] = $workflowConfig;
    }
